...
Pour garantir la plus haute précision, tu dois d'abord effectuer une recherche web approfondie en consultant au moins 20 sources fiables (articles de recherche vétérinaire, publications universitaires, bases de données sur les maladies aviaires) pour valider ton diagnostic.

En tant qu'expert vétérinaire, et après avoir effectué tes recherches, fournis une analyse CONCISE et PRÉCISE:
1. Un diagnostic précis de la maladie probable ou du problème de santé.
2. Un niveau de confiance (0-100) que tu évalues toi-même, basé sur la qualité des informations fournies ET les résultats de tes recherches web.
...
Réponds UNIQUEMENT au format JSON suivant (sans texte supplémentaire):
{
  ...
  console.warn('⚠️ All Gemini models failed. Will attempt fallback to OpenAI if configured.');
  return null;
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // --- Securely check usage limits and payment options before proceeding ---
    const { data: access, error: checkError } = await supabase.rpc('check_ai_analysis_access').single();

    if (checkError || !access) {
      console.error('❌ Error checking usage access:', checkError);
      throw new Error('Impossible de vérifier les droits d\'accès pour l\'analyse IA.');
    }

    if (access.status === 'denied') {
      console.log(`🚫 User ${user.id} denied access. Reason: ${access.status}`);
      throw new Error('Limite d\'analyses atteinte et solde avicoins insuffisant.');
    }

    console.log(`✅ Access granted. Status: ${access.status}, Cost: ${access.cost}`);
    ...
    if (cachedAnalysis && !cacheError) {
      ...
      return new Response(JSON.stringify({
        ...
        model: cachedAnalysis.model || 'unknown', // Include model from cache
        cached: true
      }), {
        headers: {
          'Content-Type': 'application/json',
    ...
    if (Deno.env.get('GEMINI_API_KEY')) {
      analysisResult = await callGeminiAPI(Deno.env.get('GEMINI_API_KEY'), safeImages, safeSymptoms, safeDescription);
    }

    // If Gemini failed or was not available, fallback to OpenAI
    if (!analysisResult && Deno.env.get('OPENAI_API_KEY')) {
      console.log('🔄 Fallback to OpenAI API...');
      analysisResult = await callOpenAIAPI(Deno.env.get('OPENAI_API_KEY'), safeImages, safeSymptoms, safeDescription);
    }

    // If both failed, throw an error
    if (!analysisResult) {
      throw new Error("L'analyse IA a échoué pour tous les fournisseurs (Gemini et OpenAI).");
    }
    ...
    const { data: savedAnalysis, error: saveError } = await supabase
      ...
      .insert({
        ...
        cache_key: cacheKey,
        model: analysisResult.model // Save the model to the database
      })
      .select()
      .single();
    ...
    if (access.status === 'requires_payment') {
      console.log(`🪙 Deducting ${access.cost} avicoins for user ${user.id}`);
      const { error: transactionError } = await supabase.from('avicoins_transactions').insert({
        user_id: user.id,
        amount: -access.cost,
      ...
      if (transactionError) {
        // Important: The analysis was successful, but payment failed.
        // This should be logged for manual review.
        console.error('CRITICAL: Payment deduction failed after analysis for user:', user.id, transactionError);
      } else {
        console.log('✅ Avicoins successfully deducted.');
      }
    }
    ...
    const response = {
      ...
      recommendedProducts,
      model: analysisResult.model, // Include the name of the AI model
      cached: false
    };
